...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    if (inkwell_uf2_block_parse(data, size, &block)) {
        if (block.has_family != ((block.flags & INKWELL_UF2_FLAG_FAMILY_ID) != 0U)) {
            fuzz_broke("a family read from a block that did not declare one");
        }
        if (block.skip != ((block.flags & INKWELL_UF2_FLAG_NOT_MAIN_FLASH) != 0U)) {
     ...
     */
    const enum inkwell_uf2_verdict open = inkwell_uf2_validate(data, size, 0U, &info);
    if (open == INKWELL_UF2_OK) {
        if (info.blocks != info.num_blocks) {
            fuzz_broke("a valid file whose block count does not match its own declaration");
        }
        if (info.blocks != size / INKWELL_UF2_BLOCK_SIZE) {
            fuzz_broke("a valid file that did not account for every record in it");
        }
        if (info.payload_bytes > (uint64_t)info.blocks * INKWELL_UF2_PAYLOAD_MAX) {
        ...
        static const uint32_t k_families[] = {INKWELL_UF2_FAMILY_NRF52840,
                                              INKWELL_UF2_FAMILY_RP2040, INKWELL_UF2_FAMILY_RP2350};
        for (size_t i = 0; i < sizeof k_families / sizeof k_families[0]; ++i) {
            const enum inkwell_uf2_verdict aimed =
                inkwell_uf2_validate(data, size, k_families[i], NULL);
            const bool matches = info.has_family && info.family_id == k_families[i];
            if (matches && aimed != INKWELL_UF2_OK) {
                fuzz_broke("a file refused for the family it declares");
            }
            if (!matches && aimed != INKWELL_UF2_WRONG_FAMILY) {
