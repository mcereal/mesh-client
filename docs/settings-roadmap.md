...
| Store and Forward | `ModuleConfig.store_forward.enabled` (+ `heartbeat`, `is_server`) | Shipped in phase 8, extended in phase 9 with the three server numbers. The section also carries the one part of the module that is not configuration: **Missed messages**, which asks a router on the mesh for the traffic that arrived while the Brick was off (`STORE_FORWARD_APP`, not an `AdminMessage`). See [`architecture.md`](architecture.md#srccorestore_forwardc--the-messages-that-arrived-while-the-client-was-off). |
