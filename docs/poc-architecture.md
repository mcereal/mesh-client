...
- UI store snapshot (discovery + handshake roster) now persists to disk so SDL/CLI backends can show cached status before BLE reconnects; CLI JSON includes `cached` metadata for automation. *(Apr 2025)*
