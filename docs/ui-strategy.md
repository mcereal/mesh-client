...
- [done] CLI backend implemented for host development; backend selection driven by `MESHCLIENT_UI_BACKEND` (defaults to CLI unless MinUI/FB backends are present).
- [done] Framebuffer fallback backend renders a lightweight HUD directly via `/dev/fb0` (trimmed bitmap font) so TrimUI builds show status even without MinUI helpers.
