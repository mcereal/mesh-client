...
| `lock` | this direct message was decrypted with our key pair rather than with a channel PSK, **and whether that key was ever proven to be theirs**. The one mark on a bubble that no word on screen repeats: on a channel still using the default key every node on the mesh holds that key, so a DM that did *not* go out PKI-encrypted was readable by all of them and nothing else distinguishes the two. A padlock is the ordinary case - a key that arrived over the air from whoever sent it; a shield is a key somebody verified out of band, which is `src/ui/trust.c`'s answer rather than the screen's. Two shapes and not two colours: at 28 px on a trailing run, two hues of one glyph are not a difference |
...
| `airtime history MINUTES` | that many minutes of the radio's once-a-minute airtime already behind us, from a fixed generator - a quiet channel with bursts and a busier second half. The only way a capture shows the Status chart a device draws after an afternoon, since the harness clock runs for seconds |
...
| `verified NAME` | mark that node's public key as proven out of band. Its own verb for `offradio`'s reason: the bit is set by the *radio* after a ceremony this side only asks for, so no press the harness can make reaches it. It is what the shield on a direct message and the "verified in person" row on a node's detail both read |
| `verify waiting\|show\|enter\|compare\|off NAME` | raise the key-verification sheet at one of its stages, or take it away. Also its own verb, and for a sharper version of the same reason: the stages are raised by ClientNotifications - the radio asking its own user something - so a scene that pressed "Verify this key" would film a toast and then nothing. The four digits and the code are invented here exactly as a message body is; what is on show is the panel |
