...
Check that `launch.sh` remains POSIX-compliant and logs to `.userdata/tg5040/logs/`. After `scripts/package.sh`, sideload the pak into `Tools/tg5040/` and verify it starts from NextUI Launcher before merging. Avoid committing user-specific paths or logs—only ship reproducible assets inside the pak tree.
