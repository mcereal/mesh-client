...
- **On-device controls:** the framebuffer HUD is five tabs (Messages, Nodes, Compose, Devices, Status). Left/Right or L1/R1 switch tabs, Up/Down move the cursor, A acts on the row (reply to a message, target a node, send a quick reply, connect to a radio), B leaves Compose, MENU quits. Quick replies come from `$HOME/.meshclient/canned.txt` (one per line) or a built-in list. Details in [`docs/device.md`](docs/device.md).
