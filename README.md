...
- **UI backend:** Set `MESHCLIENT_UI_BACKEND=auto|minui|sdl|cli|stub` to pick the renderer. The TrimUI pak defaults to the new `sdl` backend which renders a simple framebuffer HUD (with controller exit handling) when SDL2 is available; we fall back to CLI if SDL cannot initialise. The MinUI backend still emits JSON menus for `minui-list` and handles selections to request BLE connects without blocking. On desktop the placeholder helpers select the first device automatically; export `MESHCLIENT_MINUI_SELECTION=<index>` to pick a different row.
