...
- Makefile/CMake, `scripts/build.sh`, `.clang-format`, `third_party/nanopb/`.
- Pull and pin Meshtastic `.proto`, generate nanopb sources.

### Pak scaffold

- Create `/Tools/tg5040/MeshClient.pak/launch.sh` with logging + env.
...
- `scripts/package.sh` → `MeshClient.pak.zip` (or `.pakz`).
- Install via Pak Store or manual copy; validate logs, memory, and battery.

## 14) Repository Deliverables

- `README.md` (this doc)
- `/docs/transport.md` (BLE/NUS + protobuf framing details with links)
- `/Tools/tg5040/MeshClient.pak/` (ready‑to‑install)
- `scripts/build.sh`, `scripts/package.sh`
- `third_party/`:
  - `nanopb/`
  - `minui-list/`, `minui-keyboard/`, `minui-presenter/` (binaries for arm64)
