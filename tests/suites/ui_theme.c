...
MESH_TEST_CASE(ui_theme_registry_round_trips, unit) {
    MESH_TEST_FAIL_IF(inkcell_theme_count() == 0U, "no themes are registered");
    MESH_TEST_FAIL_IF(inkcell_theme_default() != inkcell_theme_at(0U),
    ...
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        MESH_TEST_FAIL_IF(inkcell_theme_by_id(theme->id) != theme,
                          "a theme does not come back under its own id");
        for (size_t j = 0; j < i; ++j) {
            MESH_TEST_FAIL_IF(strcmp(inkcell_theme_at(j)->id, theme->id) == 0,
                              "two themes share an id");
        }
    }

    MESH_TEST_FAIL_IF(inkcell_theme_at(inkcell_theme_count()) != NULL,
                      "an index past the end returned a theme");
    MESH_TEST_FAIL_IF(inkcell_theme_by_id("no-such-theme") != NULL,
                      "an unknown id returned a theme");
    MESH_TEST_FAIL_IF(inkcell_theme_by_id(NULL) != NULL, "a NULL id returned a theme");
    ...
    MESH_TEST_FAIL_IF(strcmp(inkcell_theme_default()->id, "dark") != 0,
...
MESH_TEST_CASE(ui_theme_avatar_palettes, unit) {
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        const struct inkcell_theme *theme = inkcell_theme_at(i);
        MESH_TEST_FAIL_IF(theme->avatar_count == 0U || theme->avatar_count > INKCELL_AVATAR_TINTS,
...
MESH_TEST_CASE(ui_theme_contrast_is_the_wcag_ratio, unit) {
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_contrast(white, black) != extreme,
                      "the ratio depends on the order of its arguments");
    MESH_TEST_FAIL_IF(inkcell_theme_contrast(white, white) != 1.0, "a colour on itself is not 1:1");
    record_success(test_name);
}
...
MESH_TEST_CASE(ui_theme_tone_for_load_bands, unit) {
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(0, 250, 500) != INKCELL_TONE_SUCCESS,
                      "nothing used should be good news");
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(249, 250, 500) != INKCELL_TONE_SUCCESS,
    ...
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(250, 250, 500) != INKCELL_TONE_WARNING,
                      "the warning threshold itself should warn");
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(499, 250, 500) != INKCELL_TONE_WARNING,
                      "just under the bad threshold is still a warning");
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(500, 250, 500) != INKCELL_TONE_ERROR,
                      "the bad threshold itself should be bad");
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(1000, 250, 500) != INKCELL_TONE_ERROR,
    ...
    MESH_TEST_FAIL_IF(inkcell_tone_for_load(300, 500, 250) != INKCELL_TONE_WARNING,
    ...
    for (int32_t level = 0; level <= INKCELL_ANIM_ONE; level += 50) {
        ...
        MESH_TEST_FAIL_IF(inkcell_tone_family(tone) == INKCELL_FAMILY_COUNT,
...
MESH_TEST_CASE(ui_theme_band_tone_directions, unit) {
    ...
    MESH_TEST_FAIL_IF(inkcell_band_tone(NULL, 900, INKCELL_TONE_PRIMARY) != INKCELL_TONE_PRIMARY,
                      "a reading with no band earned a tone anyway");

    MESH_TEST_FAIL_IF(inkcell_band_tone(&rising, 0, INKCELL_TONE_PRIMARY) != INKCELL_TONE_PRIMARY,
                      "a reading below every boundary did not rest in the tone it was given");
    MESH_TEST_FAIL_IF(inkcell_band_tone(&rising, 250, INKCELL_TONE_SUCCESS) != INKCELL_TONE_WARNING,
                      "the warning boundary itself did not warn");
    MESH_TEST_FAIL_IF(inkcell_band_tone(&rising, 500, INKCELL_TONE_SUCCESS) != INKCELL_TONE_ERROR,
    ...
    for (int32_t level = 250; level <= INKCELL_ANIM_ONE; level += 25) {
        MESH_TEST_FAIL_IF(inkcell_band_tone(&rising, level, INKCELL_TONE_SUCCESS) !=
    ...
    MESH_TEST_FAIL_IF(inkcell_band_tone(&falling, 87, INKCELL_TONE_SUCCESS) != INKCELL_TONE_SUCCESS,
                      "a healthy battery did not rest");
    MESH_TEST_FAIL_IF(inkcell_band_tone(&falling, 30, INKCELL_TONE_SUCCESS) != INKCELL_TONE_WARNING,
                      "the low boundary itself did not warn");
    MESH_TEST_FAIL_IF(inkcell_band_tone(&falling, 16, INKCELL_TONE_SUCCESS) != INKCELL_TONE_WARNING,
                      "just above critical stopped being a warning");
    MESH_TEST_FAIL_IF(inkcell_band_tone(&falling, 15, INKCELL_TONE_SUCCESS) != INKCELL_TONE_ERROR,
                      "the critical boundary itself was not bad");
    MESH_TEST_FAIL_IF(inkcell_band_tone(&falling, 0, INKCELL_TONE_SUCCESS) != INKCELL_TONE_ERROR,
...
MESH_TEST_CASE(ui_theme_validate_holds_the_meter_pairs, unit) {
    ...
    reason[0] = '\0';
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&flat, reason, sizeof reason),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&invisible_track, reason, sizeof reason),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&invisible_on_card, reason, sizeof reason),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&thin, reason, sizeof reason),
...
MESH_TEST_CASE(ui_theme_series_palette, unit) {
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        for (uint32_t slot = 0; slot < INKCELL_SERIES_COLORS; ++slot) {
            ...
            MESH_TEST_FAIL_IF(color.r == theme->colors[INKCELL_COLOR_BG].r &&
...
MESH_TEST_CASE(ui_theme_validate_holds_the_series_palette, unit) {
    ...
    reason[0] = '\0';
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&collapsed, reason, sizeof reason),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&invisible, reason, sizeof reason),
...
MESH_TEST_CASE(ui_theme_validate_rejects_an_unreadable_palette, unit) {
    ...
    char reason[128];
    reason[0] = '\0';
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&broken, reason, sizeof reason),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&no_font, reason, sizeof reason),
...
MESH_TEST_CASE(ui_theme_states_its_geometry, unit) {
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        MESH_TEST_FAIL_IF(metrics->shape[INKCELL_SHAPE_NONE] != 0U,
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_radius(shaped, INKCELL_SHAPE_FULL, scale) < 4096,
    ...
    /* A NULL theme resolves to the default, like every other lookup here, and a shape outside
       ...
       a corner of garbage. */
    MESH_TEST_FAIL_IF(inkcell_theme_radius(NULL, INKCELL_SHAPE_MD, scale) !=
    ...
    reason[0] = '\0';
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&swallowed, reason, sizeof reason),
    ...
    reason[0] = '\0';
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&edgeless, reason, sizeof reason),
...
MESH_TEST_CASE(ui_theme_states_its_type_scale, unit) {
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        for (int scale = INKCELL_SCALE_MIN; scale <= INKCELL_SCALE_MAX; ++scale) {
            ...
            MESH_TEST_FAIL_IF(title < INKCELL_SCALE_MIN || title > INKCELL_SCALE_MAX,
                              "a title scale fell outside the drawable range");
            MESH_TEST_FAIL_IF(label < INKCELL_SCALE_MIN || label > INKCELL_SCALE_MAX,
            ...
            MESH_TEST_FAIL_IF(body != inkcell_theme_clamp_scale(theme, scale),
        ...
        MESH_TEST_FAIL_IF(inkcell_theme_type_scale(theme, INKCELL_TYPE_TITLE, INKCELL_SCALE_MAX) !=
                              INKCELL_SCALE_MAX,
                          "a title at the maximum scale did not collapse onto it");
        MESH_TEST_FAIL_IF(inkcell_theme_type_scale(theme, INKCELL_TYPE_LABEL, INKCELL_SCALE_MIN) !=
                              INKCELL_SCALE_MIN,
...
MESH_TEST_CASE(ui_theme_states_its_spacing, unit) {
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        MESH_TEST_FAIL_IF(inkcell_theme_space(theme, INKCELL_SPACE_NONE, scale) != 0,
        ...
        MESH_TEST_FAIL_IF(inkcell_theme_space(theme, INKCELL_SPACE_MD, INKCELL_SCALE_MAX) <
...
MESH_TEST_CASE(ui_theme_states_its_motion, unit) {
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        for (int motion = INKCELL_MOTION_SHORT; motion <= INKCELL_MOTION_LONG; ++motion) {
             ...
             */
            MESH_TEST_FAIL_IF(motion > INKCELL_MOTION_SHORT && ms <= previous,
    ...
    const struct inkcell_theme *theme = inkcell_theme_default();
    MESH_TEST_FAIL_IF(inkcell_theme_motion(theme, (enum inkcell_motion) - 1) != 0U,
                      "a negative motion token read something");
    MESH_TEST_FAIL_IF(inkcell_theme_motion(theme, INKCELL_MOTION_COUNT) != 0U,
                      "a motion token past the end read something");
    /* NULL is the default theme, as it is everywhere else in this header. */
    MESH_TEST_FAIL_IF(inkcell_theme_motion(NULL, INKCELL_MOTION_SHORT) !=
...
MESH_TEST_CASE(ui_theme_cycles_through_every_theme, unit) {
    ...
    MESH_TEST_FAIL_IF(theme != inkcell_theme_default(),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_next(&stray) != inkcell_theme_default(),
...
MESH_TEST_CASE(ui_theme_resolves_ids_and_the_environment, unit) {
    MESH_TEST_FAIL_IF(inkcell_theme_resolve("light") != inkcell_theme_by_id("light"),
                      "a known id did not resolve to its theme");
    MESH_TEST_FAIL_IF(inkcell_theme_resolve("no-such-theme") != inkcell_theme_default(),
                      "an unknown id did not resolve to the default");
    MESH_TEST_FAIL_IF(inkcell_theme_resolve("") != inkcell_theme_default(),
                      "an empty id did not resolve to the default");
    MESH_TEST_FAIL_IF(inkcell_theme_resolve(NULL) != inkcell_theme_default(),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_env() != NULL, "an unset MESHCLIENT_THEME named a theme");
    MESH_TEST_FAIL_IF(inkcell_theme_from_env() != inkcell_theme_default(),
                      "an unset MESHCLIENT_THEME did not fall back to the default");

    (void)setenv("MESHCLIENT_THEME", "light", 1);
    MESH_TEST_FAIL_IF(inkcell_theme_env() != inkcell_theme_by_id("light"),
    ...
    MESH_TEST_FAIL_IF(inkcell_theme_env() != NULL, "an unknown MESHCLIENT_THEME named a theme");
    MESH_TEST_FAIL_IF(inkcell_theme_from_env() != inkcell_theme_default(),
...
MESH_TEST_CASE(ui_theme_scale_is_clamped, unit) {
    const struct inkcell_theme *theme = inkcell_theme_default();
    MESH_TEST_FAIL_IF(inkcell_theme_clamp_scale(theme, 0) != inkcell_theme_scale(theme),
                      "scale 0 is not the theme's own");
    MESH_TEST_FAIL_IF(inkcell_theme_clamp_scale(theme, 99) != INKCELL_SCALE_MAX,
                      "an absurd scale was not clamped down");
    MESH_TEST_FAIL_IF(inkcell_theme_clamp_scale(theme, -4) != inkcell_theme_scale(theme),
                      "a negative scale is not the theme's own");
    MESH_TEST_FAIL_IF(inkcell_theme_clamp_scale(NULL, 1) != INKCELL_SCALE_MIN,
...
MESH_TEST_CASE(ui_theme_fonts_measure, unit) {
    MESH_TEST_FAIL_IF(inkcell_font_count() == 0U, "no fonts are registered");
    for (size_t i = 0; i < inkcell_font_count(); ++i) {
        ...
        MESH_TEST_FAIL_IF(inkcell_font_by_id(font->id) != font,
                          "a font does not come back under its own id");
        MESH_TEST_FAIL_IF(font->width == 0U || font->width > INKCELL_GLYPH_MAX_WIDTH,
                          "a font is wider than the glyph buffer");
        MESH_TEST_FAIL_IF(font->height == 0U || font->height > INKCELL_GLYPH_MAX_HEIGHT,
        ...
        MESH_TEST_FAIL_IF(font->master_w == 0U || font->master_w > INKCELL_GLYPH_MASTER_MAX_WIDTH,
                          "a font's master is wider than the glyph buffer");
        MESH_TEST_FAIL_IF(font->master_h == 0U || font->master_h > INKCELL_GLYPH_MASTER_MAX_HEIGHT,
                          "a font's master is taller than the glyph buffer");
        MESH_TEST_FAIL_IF(font->sampling != INKCELL_FONT_PIXEL &&
        ...
        MESH_TEST_FAIL_IF(!inkcell_font_glyph(font, (uint32_t)'A', &glyph),
                          "the font has no capital A");
        MESH_TEST_FAIL_IF(inkcell_font_glyph(font, 0x10FFFDU, &glyph),
    ...
    struct inkcell_theme no_font = *inkcell_theme_default();
    no_font.font_id = "not-a-font";
    MESH_TEST_FAIL_IF(inkcell_theme_font(&no_font) != inkcell_font_default(),
                      "an unknown font id did not fall back to the default");
    MESH_TEST_FAIL_IF(inkcell_theme_font(NULL) == NULL, "a NULL theme resolved to no font");
    record_success(test_name);
}
...
MESH_TEST_CASE(ui_theme_switch_repaints_the_frame, unit) {
    ...
    MESH_TEST_FAIL_IF_CLEANUP(reference == NULL, inkcell_capture_close(capture);
...
#define THEME_CLEANUP                                                                              \
    ...
    for (size_t i = 1U; i < inkcell_theme_count(); ++i) {
        ...
        MESH_TEST_FAIL_IF_CLEANUP(
            !ground_is(pixels, stride, width, height, inkcell_theme_color(theme, INKCELL_COLOR_BG)),
...
#define SNAPSHOT_CLEANUP                                                                           \
    ...
    MESH_TEST_FAIL_IF_CLEANUP(
        !ground_is(pixels, stride, width, height,
                   inkcell_theme_color(inkcell_theme_default(), INKCELL_COLOR_BG)),
    ...
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
        ...
        snprintf(snapshot.settings.client.theme, sizeof snapshot.settings.client.theme, "%s",
                 theme->id);
        inkcell_capture_render(capture, &snapshot);
        MESH_TEST_FAIL_IF_CLEANUP(inkcell_capture_theme(capture) != theme, SNAPSHOT_CLEANUP,
     ...
     */
    const struct inkcell_theme *const last = inkcell_capture_theme(capture);
    ...
    const struct inkcell_theme *before = inkcell_capture_theme(capture);
    snprintf(snapshot.settings.client.theme, sizeof snapshot.settings.client.theme, "%s",
             "solarized");
    inkcell_capture_render(capture, &snapshot);
    MESH_TEST_FAIL_IF_CLEANUP(inkcell_capture_theme(capture) != before, SNAPSHOT_CLEANUP,
...
MESH_TEST_CASE(ui_theme_carries_its_own_scale, unit) {
    ...
    MESH_TEST_FAIL_IF_CLEANUP(at_theme_scale == NULL, inkcell_capture_close(capture);
                              mesh_ui_store_shutdown(&store), "out of memory");
...
#define SCALE_CLEANUP                                                                              \
    ...
    inkcell_capture_set_scale(capture, INKCELL_SCALE_MAX);
    inkcell_capture_render(capture, &snapshot);
...
/*
 * The families are an index, not a switch: inkcell_family_role() multiplies, and the enum is
...
MESH_TEST_CASE(ui_theme_family_slots_are_an_index, unit) {
    for (int f = 0; f < INKCELL_FAMILY_COUNT; ++f) {
        ...
        for (int sl = 0; sl < INKCELL_SLOT_COUNT; ++sl) {
            const enum inkcell_color role = inkcell_family_role(family, (enum inkcell_slot)sl);
            MESH_TEST_FAIL_IF(role >= INKCELL_COLOR_COUNT, "a family slot left the palette");
            MESH_TEST_FAIL_IF((int)role != (int)INKCELL_COLOR_PRIMARY + f * INKCELL_SLOT_COUNT + sl,
        ...
        MESH_TEST_FAIL_IF(inkcell_tone_family(inkcell_family_tone(family)) != family,
                          "a family did not survive the trip through its tone");
        MESH_TEST_FAIL_IF(inkcell_tone_role(inkcell_family_tone(family)) !=
                              inkcell_family_role(family, INKCELL_SLOT_BASE),
                          "a family tone did not resolve to its own base");
    }

    /* The neutral three name no family - which is the answer inkcell_fb_draw_meter() acts on. */
    MESH_TEST_FAIL_IF(inkcell_tone_family(INKCELL_TONE_NORMAL) != INKCELL_FAMILY_COUNT,
                      "the normal tone claimed a family");
    MESH_TEST_FAIL_IF(inkcell_tone_family(INKCELL_TONE_DIM) != INKCELL_FAMILY_COUNT,
                      "the dim tone claimed a family");
    MESH_TEST_FAIL_IF(inkcell_tone_family(INKCELL_TONE_STRONG) != INKCELL_FAMILY_COUNT,
    ...
    MESH_TEST_FAIL_IF(inkcell_family_role((enum inkcell_family) - 1, INKCELL_SLOT_BASE) !=
                          INKCELL_COLOR_PRIMARY,
                      "an out-of-range family did not fall back to the primary");
    MESH_TEST_FAIL_IF(inkcell_family_role(INKCELL_FAMILY_ERROR, (enum inkcell_slot)99) !=
                          INKCELL_COLOR_ERROR,
...
MESH_TEST_CASE(ui_theme_state_layer_moves_towards_the_ink, unit) {
    const struct inkcell_rgb fill = {10, 20, 30};
    const struct inkcell_rgb ink = {250, 240, 230};

    const struct inkcell_rgb rest = inkcell_theme_state_layer(fill, ink, INKCELL_STATE_REST);
...
/*
 * inkcell_theme_paint() exists so that a fill and the label on it can only ever be chosen
...
MESH_TEST_CASE(ui_theme_paint_returns_a_validated_pair, unit) {
    ...
    for (size_t i = 0; i < inkcell_theme_count(); ++i) {
            ...
            for (size_t sl = 0; sl < sizeof k_slots / sizeof k_slots[0]; ++sl) {
                for (size_t st = 0; st < sizeof k_states / sizeof k_states[0]; ++st) {
                    const struct inkcell_paint paint = inkcell_theme_paint(
                        theme, (enum inkcell_family)f, k_slots[sl], k_states[st]);
                    ...
                    if (k_slots[sl] == INKCELL_SLOT_BASE) {
                        const struct inkcell_rgb base =
                            inkcell_theme_family(theme, (enum inkcell_family)f, INKCELL_SLOT_BASE);
                        MESH_TEST_FAIL_IF(paint.fill.r != base.r || paint.fill.g != base.g ||
                                              paint.fill.b != base.b,
                                          "a state layer was applied to a family's base");
                    }
                    const double ratio = inkcell_theme_contrast(paint.ink, paint.fill);
                    char why[160];
        ...
        const struct inkcell_paint by_fill = inkcell_theme_paint(
            theme, INKCELL_FAMILY_ERROR, INKCELL_SLOT_CONTAINER, INKCELL_STATE_REST);
        const struct inkcell_paint by_ink = inkcell_theme_paint(
            theme, INKCELL_FAMILY_ERROR, INKCELL_SLOT_ON_CONTAINER, INKCELL_STATE_REST);
...
MESH_TEST_CASE(ui_theme_validate_holds_the_family_contracts, unit) {
    ...
    struct inkcell_theme collapsed = *inkcell_theme_default();
    collapsed.colors[INKCELL_COLOR_WARNING] = collapsed.colors[INKCELL_COLOR_ERROR];
    reason[0] = '\0';
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&collapsed, reason, sizeof reason),
    ...
    struct inkcell_theme invisible_marker = *inkcell_theme_default();
    invisible_marker.colors[INKCELL_COLOR_TERTIARY] =
        invisible_marker.colors[INKCELL_COLOR_SURFACE_SEL];
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&invisible_marker, reason, sizeof reason),
    ...
    struct inkcell_theme flat_container = *inkcell_theme_default();
    flat_container.colors[INKCELL_COLOR_ON_SUCCESS_CONTAINER] =
        flat_container.colors[INKCELL_COLOR_SUCCESS_CONTAINER];
    MESH_TEST_FAIL_IF(inkcell_theme_validate(&flat_container, reason, sizeof reason),
