...
MESH_TEST_CASE(session_node_detail_ingest, unit) {
    ...
    if (!mesh_test_session_feed_from_radio(&session, &sync)) {
        record_failure(test_name, "encode node_info failed");
        return;
    }
    ...
    if (!pb_encode(&stream, meshtastic_Position_fields, &position) ||
        !mesh_test_session_feed_app_packet(&session, 0x4001U, meshtastic_PortNum_POSITION_APP,
    ...
    if (!pb_encode(&stream, meshtastic_Telemetry_fields, &telemetry) ||
        !mesh_test_session_feed_app_packet(&session, 0x4001U, meshtastic_PortNum_TELEMETRY_APP,
                                           payload, stream.bytes_written)) {
    ...
    if (!pb_encode(&stream, meshtastic_User_fields, &user) ||
        !mesh_test_session_feed_app_packet(&session, 0x4002U, meshtastic_PortNum_NODEINFO_APP,
                                           payload, stream.bytes_written)) {
...
MESH_TEST_CASE(session_local_stats, unit) {
    ...
    if (!mesh_test_session_feed_app_packet(&session, 0x7002U, meshtastic_PortNum_TELEMETRY_APP,
                                           payload, payload_len)) {
    ...
    if (!mesh_test_session_feed_app_packet(&session, 0x7001U, meshtastic_PortNum_TELEMETRY_APP,
                                           payload, payload_len)) {
    ...
    stream = pb_ostream_from_buffer(payload, sizeof payload);
    if (!pb_encode(&stream, meshtastic_Telemetry_fields, &telemetry) ||
        !mesh_test_session_feed_app_packet(&session, 0x7001U, meshtastic_PortNum_TELEMETRY_APP,
                                           payload, stream.bytes_written)) {
...
MESH_TEST_CASE(session_traceroute, unit) {
    ...
    if (!mesh_test_session_feed_from_radio(&session, &stray)) {
    ...
    if (!mesh_test_session_feed_from_radio(&session, &reply)) {
...
MESH_TEST_CASE(session_node_actions, unit) {
    ...
    if (!mesh_test_session_feed_from_radio(&session, &peer)) {
        record_failure(test_name, "encode node_info failed");
        return;
    }
    ...
    if (!mesh_test_session_feed_from_radio(&session, &owner)) {
