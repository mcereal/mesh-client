...
MESH_TEST_CASE(anim_easings_pin_both_ends, unit) {
    for (int ease = 0; ease < INKCELL_EASE_COUNT; ++ease) {
        MESH_TEST_FAIL_IF(inkcell_ease((enum inkcell_ease)ease, 0) != 0,
                          "every curve should start at 0");
        MESH_TEST_FAIL_IF(inkcell_ease((enum inkcell_ease)ease, INKCELL_ANIM_ONE) !=
                              INKCELL_ANIM_ONE,
        ...
        MESH_TEST_FAIL_IF(inkcell_ease((enum inkcell_ease)ease, -500) != 0,
                          "a negative progress should clamp to 0");
        MESH_TEST_FAIL_IF(inkcell_ease((enum inkcell_ease)ease, 5 * INKCELL_ANIM_ONE) !=
                              INKCELL_ANIM_ONE,
    ...
    MESH_TEST_FAIL_IF(inkcell_ease(INKCELL_EASE_IN_OUT, INKCELL_ANIM_ONE / 2) !=
                          INKCELL_ANIM_ONE / 2,
    ...
    MESH_TEST_FAIL_IF(early + late != INKCELL_ANIM_ONE, "ease-in-out should be symmetric");
...
MESH_TEST_CASE(anim_value_walks_from_start_to_target, unit) {
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 1000U) != 0, "a set value should sit where it is");
    MESH_TEST_FAIL_IF(inkcell_anim_active(&anim, 1000U), "nothing set should be in flight");

    inkcell_anim_to(&anim, 1000U, INKCELL_ANIM_ONE, 200U, INKCELL_EASE_LINEAR);
    MESH_TEST_FAIL_IF(!inkcell_anim_active(&anim, 1000U), "it should be in flight at the start");
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 1000U) != 0, "it should start where it was");
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 1100U) != INKCELL_ANIM_ONE / 2,
                      "linear should be halfway at half the duration");
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 1200U) != INKCELL_ANIM_ONE,
                      "it should land exactly on its target");
    MESH_TEST_FAIL_IF(inkcell_anim_active(&anim, 1200U),
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 900000U) != INKCELL_ANIM_ONE,
...
MESH_TEST_CASE(anim_reversal_starts_from_where_it_got_to, unit) {
    ...
    MESH_TEST_FAIL_IF(midway != INKCELL_ANIM_ONE / 2, "half a duration in should be half way");

    inkcell_anim_to(&anim, 1100U, 0, 200U, INKCELL_EASE_LINEAR);
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 1100U) != midway,
                      "a reversal should begin from where the knob actually is");
    MESH_TEST_FAIL_IF(inkcell_anim_value(&anim, 1300U) != 0,
...
MESH_TEST_CASE(anim_table_adopts_then_animates, unit) {
    ...
    MESH_TEST_FAIL_IF(first != INKCELL_ANIM_ONE, "a control should be drawn at its value on sight");
    MESH_TEST_FAIL_IF(inkcell_anim_table_active(&table, 1000U),
    ...
    MESH_TEST_FAIL_IF(flipped != INKCELL_ANIM_ONE, "a change should start from the old value");
    MESH_TEST_FAIL_IF(!inkcell_anim_table_active(&table, 1000U),
                      "a change should ask for more frames");
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 7U, 1100U, 0, 200U, INKCELL_EASE_LINEAR) !=
                          INKCELL_ANIM_ONE / 2,
                      "the table should carry the animation forward across frames");
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 7U, 1200U, 0, 200U, INKCELL_EASE_LINEAR) != 0,
                      "and should land on the target");
    MESH_TEST_FAIL_IF(inkcell_anim_table_active(&table, 1200U),
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 7U, 1300U, 0, 200U, INKCELL_EASE_LINEAR) != 0,
...
MESH_TEST_CASE(anim_table_ignores_the_null_id, unit) {
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 0U, 1000U, INKCELL_ANIM_ONE, 200U,
                                         INKCELL_EASE_OUT) != INKCELL_ANIM_ONE,
                      "an unkeyed control should draw at its value");
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 0U, 1000U, 0, 200U, INKCELL_EASE_OUT) != 0,
...
MESH_TEST_CASE(anim_table_evicts_the_least_recently_drawn, unit) {
    ...
    now += 10U;
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 999U, now, INKCELL_ANIM_ONE, 200U,
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 1U, now, INKCELL_ANIM_ONE, 200U,
                                         INKCELL_EASE_LINEAR) != INKCELL_ANIM_ONE,
...
MESH_TEST_CASE(anim_loop_runs_a_sawtooth_off_the_clock, unit) {
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U, 1000U) != 0,
                      "a loop should begin at 0 whenever it is first drawn");
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U + 250U, 1000U) != INKCELL_ANIM_ONE / 4,
                      "a quarter of the period in should be a quarter of the way along");
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U + 750U, 1000U) !=
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U + 1000U, 1000U) != 0,
                      "a full period should be back at the start");
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U + 7250U, 1000U) != INKCELL_ANIM_ONE / 4,
    ...
    MESH_TEST_FAIL_IF(!inkcell_anim_table_active(&table, 987654U + 7250U),
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_loop(NULL, 1U, 1000U, 1000U) != 0,
                      "a NULL table should read 0 rather than crash");
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 0U, 1000U, 1000U) != 0,
                      "an id of 0 is not a key, exactly as it is not for a transition");
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 1000U, 0U) != 0,
...
MESH_TEST_CASE(anim_loop_and_track_do_not_bleed_into_each_other, unit) {
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_track(&table, 7U, 1600U, INKCELL_ANIM_ONE / 2, 200U,
                                         INKCELL_EASE_OUT) != INKCELL_ANIM_ONE / 2,
                      "a loop turning into a reading should adopt it, not slide from a sawtooth");
    MESH_TEST_FAIL_IF(inkcell_anim_table_active(&table, 1600U + INKCELL_ANIM_LOOP_STALE_MS + 1U),
    ...
    MESH_TEST_FAIL_IF(inkcell_anim_loop(&table, 7U, 5000U, 1000U) != 0,
...
MESH_TEST_CASE(anim_null_arguments_are_safe, unit) {
    MESH_TEST_FAIL_IF(inkcell_anim_value(NULL, 1000U) != 0, "a NULL animation should read 0");
    MESH_TEST_FAIL_IF(inkcell_anim_active(NULL, 1000U), "a NULL animation should not be active");
    MESH_TEST_FAIL_IF(inkcell_anim_table_active(NULL, 1000U), "a NULL table should not be active");
    MESH_TEST_FAIL_IF(inkcell_anim_track(NULL, 1U, 1000U, INKCELL_ANIM_ONE, 200U,
                                         INKCELL_EASE_OUT) != INKCELL_ANIM_ONE,
