 ...
 * a table whose %-specifiers no longer match the English is rejected by inkcell_i18n_validate()
 * rather than reaching a vsnprintf that would read the wrong argument off the stack.
 *
...
MESH_TEST_CASE(i18n_catalog_is_complete, unit) {
    ...
    for (int id = 0; id < (int)MESH_STR_COUNT; ++id) {
        ...
        if (which != INKCELL_STR_NONE && text[0] == '\0') {
            char reason[128];
            snprintf(reason, sizeof reason, "%s is empty and is not INKCELL_STR_NONE", name);
...
MESH_TEST_CASE(i18n_shipped_locales_validate, unit) {
    MESH_TEST_FAIL_IF(inkcell_i18n_locale_count() == 0U, "no locales are registered");
    MESH_TEST_FAIL_IF(inkcell_i18n_locale_at(0U) != inkcell_i18n_locale_english(),
                      "English is not the first locale");
    MESH_TEST_FAIL_IF(inkcell_i18n_locale_english()->table != NULL,
                      "English has a table of its own instead of being the fallback");

    for (size_t i = 0; i < inkcell_i18n_locale_count(); ++i) {
        const struct inkcell_i18n_locale *locale = inkcell_i18n_locale_at(i);
        MESH_TEST_FAIL_IF(locale == NULL, "a registered locale is NULL");
        MESH_TEST_FAIL_IF(inkcell_i18n_locale_by_id(locale->id) != locale,
                          "a locale does not come back under its own id");
        for (size_t j = 0; j < i; ++j) {
            MESH_TEST_FAIL_IF(strcmp(inkcell_i18n_locale_at(j)->id, locale->id) == 0,
                              "two locales share an id");
        }
        char reason[192];
        if (!inkcell_i18n_validate(locale, reason, sizeof reason)) {
            record_failure(test_name, reason);
            return;
        }
    }

    MESH_TEST_FAIL_IF(inkcell_i18n_locale_at(inkcell_i18n_locale_count()) != NULL,
                      "an index past the end returned a locale");
    MESH_TEST_FAIL_IF(inkcell_i18n_locale_by_id("no-such-language") != NULL,
                      "an unknown id returned a locale");
    MESH_TEST_FAIL_IF(inkcell_i18n_locale_by_id(NULL) != NULL, "a NULL id returned a locale");
    record_success(test_name);
}
...
MESH_TEST_CASE(i18n_partial_translation_falls_back, unit) {
    ...
    MESH_TEST_FAIL_IF(strcmp(inkcell_str_in(&locale, MESH_STR_TAB_NODES), "Knoten") != 0,
                      "a translated entry did not come back translated");
    MESH_TEST_FAIL_IF(
        strcmp(inkcell_str_in(&locale, MESH_STR_TAB_MESSAGES),
               inkcell_str_in(inkcell_i18n_locale_english(), MESH_STR_TAB_MESSAGES)) != 0,
        "an untranslated entry did not fall back to English");

    char reason[192];
    MESH_TEST_FAIL_IF(!inkcell_i18n_validate(&locale, reason, sizeof reason),
                      "a partial translation with matching formats was rejected");
    record_success(test_name);
}

/* The case inkcell_i18n_validate() exists for: a translation that drops or retypes an argument
   is a crash waiting for the row that uses it, not a typo. */
MESH_TEST_CASE(i18n_validate_rejects_bad_formats, unit) {
    struct inkcell_i18n_locale locale = {
        .id = "test",
        .name = "Test",
        .table = k_partial,
        .plural_form = NULL,
    ...
    MESH_TEST_FAIL_IF(inkcell_i18n_validate(NULL, reason, sizeof reason), "NULL was accepted");
    record_success(test_name);
}
...
MESH_TEST_CASE(i18n_format_and_plural, unit) {
    ...
    inkcell_str_format(out, sizeof out, MESH_STR_STATUS_NODEDB_ONLINE, 12U, 3U);
    ...
    char tiny[8];
    inkcell_str_format(tiny, sizeof tiny, MESH_STR_STATUS_NODEDB_ONLINE, 12U, 3U);
    MESH_TEST_FAIL_IF(tiny[sizeof tiny - 1U] != '\0', "a truncated format was not terminated");
    MESH_TEST_FAIL_IF(inkcell_str_format(NULL, 0U, MESH_STR_TAB_NODES) != 0,
                      "formatting into no buffer did not return 0");

    /* English: one for 1, other for everything else - including 0. */
    MESH_TEST_FAIL_IF(strcmp(inkcell_str_plural(MESH_STR_TOAST_FORGOT_NODES_ONE, 1U),
                             "Forgot %d node; pins kept") != 0,
                      "the singular form was not chosen for 1");
    MESH_TEST_FAIL_IF(strcmp(inkcell_str_plural(MESH_STR_TOAST_FORGOT_NODES_ONE, 0U),
                             "Forgot %d nodes; pins kept") != 0,
                      "the plural form was not chosen for 0");
    MESH_TEST_FAIL_IF(strcmp(inkcell_str_plural(MESH_STR_TOAST_FORGOT_NODES_ONE, 7U),
                             "Forgot %d nodes; pins kept") != 0,
                      "the plural form was not chosen for 7");

    inkcell_str_format_plural(out, sizeof out, MESH_STR_TOAST_FORGOT_NODES_ONE, 1U, 1);
    MESH_TEST_FAIL_IF(strcmp(out, "Forgot 1 node; pins kept") != 0,
                      "a formatted singular came out wrong");
    inkcell_str_format_plural(out, sizeof out, MESH_STR_TOAST_FORGOT_NODES_ONE, 4U, 4);
...
MESH_TEST_CASE(i18n_plural_rule_is_the_locales, unit) {
    const struct inkcell_i18n_locale english = {
        .id = "en", .name = "English", .table = NULL, .plural_form = NULL};
    const struct inkcell_i18n_locale other = {
    ...
    /* INKCELL_STR_PLURAL_FORMS bounds every rule, so a rule that returns more than the entry has
       cannot walk off the end of the table. */
    MESH_TEST_FAIL_IF(INKCELL_STR_PLURAL_FORMS < 2, "there is no room for a plural");
    record_success(test_name);
}
 ...
 *
 * inkcell_str_in(english, id) is the idiom for it, and the case that matters is the one where the
...
MESH_TEST_CASE(i18n_english_is_reachable_from_any_locale, unit) {
    ...
    MESH_TEST_FAIL_IF(
        strcmp(inkcell_str_in(&locale, MESH_STR_ACK_NO_ROUTE), "keine Route zu diesem Knoten") != 0,
        "the translation did not come back for the translated locale");
    MESH_TEST_FAIL_IF(strcmp(inkcell_str_in(inkcell_i18n_locale_english(), MESH_STR_ACK_NO_ROUTE),
...
MESH_TEST_CASE(i18n_locales_name_themselves, unit) {
    ...
    MESH_TEST_FAIL_IF(strcmp(inkcell_i18n_locale_english()->name, "English") != 0,
...
MESH_TEST_CASE(i18n_locale_selection, unit) {
    const struct inkcell_i18n_locale *before = inkcell_i18n_locale();
    MESH_TEST_FAIL_IF(before == NULL, "there is no current locale");

    MESH_TEST_FAIL_IF(!inkcell_i18n_set_locale("en"), "English could not be selected");
    MESH_TEST_FAIL_IF(inkcell_i18n_locale() != inkcell_i18n_locale_english(),
                      "selecting English did not take");
    MESH_TEST_FAIL_IF(inkcell_i18n_set_locale("no-such-language"),
                      "an unknown language was accepted");
    MESH_TEST_FAIL_IF(inkcell_i18n_locale() != inkcell_i18n_locale_english(),
                      "a rejected selection changed the locale anyway");
    MESH_TEST_FAIL_IF(inkcell_i18n_set_locale(NULL), "NULL was accepted as a language");
...
MESH_TEST_CASE(i18n_spanish_catalog, unit) {
    const struct inkcell_i18n_locale *spanish = inkcell_i18n_locale_by_id("es");
    ...
    for (int id = 0; id < (int)MESH_STR_COUNT; ++id) {
         ...
         * are technical prose about radio behaviour, and inkcell_str_in() already falls back to
        ...
        if (spanish->table[id] == NULL ||
            (id != INKCELL_STR_NONE && spanish->table[id][0] == '\0')) {
    ...
    for (size_t i = 0; i < sizeof letters / sizeof letters[0]; ++i) {
        MESH_TEST_FAIL_IF(!inkcell_font5x7_has_glyph(letters[i]), "a Spanish glyph is missing");
    }
    const char *before = inkcell_i18n_locale()->id;
    mesh_ui_canned_reset();
    (void)inkcell_i18n_set_locale("es");
    char one[96], zero[96], many[96];
    inkcell_str_format_plural(one, sizeof one, MESH_STR_ACTION_FORGET_COUNT_ONE, 1U, 1U);
    inkcell_str_format_plural(zero, sizeof zero, MESH_STR_ACTION_FORGET_COUNT_ONE, 0U, 0U);
    inkcell_str_format_plural(many, sizeof many, MESH_STR_ACTION_FORGET_COUNT_ONE, 3U, 3U);
    const bool translated = strcmp(inkcell_str(MESH_STR_TAB_SETTINGS), "Ajustes") == 0 &&
                            strcmp(mesh_ui_canned_text(4U), "¿Dónde estás?") == 0 &&
                            strcmp(inkcell_str(MESH_STR_DATE_WED), "mié") == 0;
...
MESH_TEST_CASE(i18n_spanish_preference_and_environment, unit) {
    ...
    for (size_t i = 0; i < sizeof tags / sizeof tags[0]; ++i) {
        ...
        inkcell_i18n_init_with_preference("en");
        if (strcmp(inkcell_i18n_locale()->id, "es") != 0 || !inkcell_i18n_is_overridden()) {
    ...
    inkcell_i18n_init_with_preference("en");
    if (strcmp(inkcell_i18n_locale()->id, "en") != 0 || inkcell_i18n_is_overridden()) {
        failure = "saved English must override system Spanish without locking the picker";
    }
    inkcell_i18n_init_with_preference("unknown");
    ...
    inkcell_i18n_init_with_preference("");
    ...
    inkcell_i18n_init_with_preference("es");
    if (strcmp(inkcell_i18n_locale()->id, "en") != 0) {
