...
Verified 2026-09-13: 635 unit tests, all passing, zero compiler warnings - under the host
...
| Press latency | `src/ui/latency.c`, `include/mesh/ui/latency.h` | how long a press takes to reach the panel, measured from the kernel's own stamp on the evdev event to the return of `FBIOPAN_DISPLAY` - plus what the frame between them was made of. Off unless `--trace-latency` or `MESHCLIENT_LATENCY_TRACE` asks; histograms rather than samples, because a fill frame is twenty times as common as a press. `devtools/input_inject` is the uinput pad that drives a repeatable run |
...
- **Half of a press is the panel, and the map's drawing is free because of it.** Measured
  ...
  panel in 26 ms with a basemap and twenty tiles under it, and 27 ms on a list of nodes with
  ...
  all**, only CPU. It is a battery argument, and the roadmap's open item says so.
- **A key repeat is deliberately not counted by the latency probe.** Repeat is generated by
  `input.c`'s own timerfd - the d-pad is an absolute axis and the kernel never repeats it - so a
  held direction reaches the store with no evdev event behind it and no kernel stamp to measure
  from. Counted from the moment the probe sees it, every press of a held pan would report a
  queueing delay of zero, which is the fill loop's worst case reported as its best.
