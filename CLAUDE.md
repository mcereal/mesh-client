...
| App framework | `third_party/inkstand/` - the plumbing any app on this stack would write again; today the frame scheduler under `controller.c` and the snackbar's queue and the two-answer dialog under the nav (`inkstand/nav/`), the control socket `make ui-drive` talks to (`inkstand/app/control.h`), the scene runner under `ui_capture`, the cache's key table and field reader, the journal both logs on the card stand on, the recently-used list under `preferences.c`'s radios and devices (`inkstand/persist/`), and under the settings model the text codec, the preset step, track and choice walk, and the field descriptor `k_fields` rows begin with (`inkstand/form/`). Its `docs/extraction.md` is the map of what comes down next |
...
| Tables the UI reads | `src/ui/tables/` - `actions.c` (button verbs), `status.c` (card verbs), `protocols.c` (which features each mesh protocol has - see [`docs/protocols.md`](docs/protocols.md#what-the-ui-offers-per-protocol)), `help.c`, `devices.c`, `nodes.c`, `delivery.c`, `trust.c`, `chrome.c`, `trend.c` (the airtime chart; the frame around it is inkcell's), `duration.c`, `units.c` (metric/imperial lengths) |
