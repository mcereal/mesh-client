 ...
 * anything.
 */
static void mqtt_fail(struct mesh_mqtt_proxy *proxy, enum mesh_str_id text, ...) {
    va_list args;
    va_start(args, text);
    if (proxy->last_error[0] == '\0') {
        (void)mesh_str_vformat(proxy->last_error, sizeof proxy->last_error, text, args);
    }
    va_end(args);
    ...
    mesh_log_warn("mqtt", "%s; retrying in %llums", proxy->last_error, (unsigned long long)delay);
}
...
static void mqtt_pump_subscribes(struct mesh_mqtt_proxy *proxy) {
    ...
    if (len < 0) {
        mesh_log_warn("mqtt", "Cannot subscribe to %s: %d", proxy->filters[proxy->filter_sent],
                      len);
        proxy->filter_sent++;
    ...
    if (queued == -ENOSPC) {
        ...
        mesh_log_warn("mqtt", "No room to send a subscription");
        return;
    }
    if (queued < 0) {
...
static bool mqtt_on_connack(struct mesh_mqtt_proxy *proxy, const uint8_t *body, size_t len) {
    ...
    mesh_log_info("mqtt", "Connected to %s as %s", proxy->host, proxy->config.client_id);
...
static bool mqtt_on_suback(struct mesh_mqtt_proxy *proxy, const uint8_t *body, size_t len) {
    ...
    if (id != proxy->subscribe_id) {
        ...
        mesh_log_debug("mqtt", "Unexpected SUBACK %u", (unsigned)id);
        return true;
    }
    if (code == MESH_MQTT_SUBACK_FAILURE) {
         ...
         */
        mesh_log_warn("mqtt", "%s refused the subscription to %s", proxy->host,
...
static bool mqtt_on_publish(struct mesh_mqtt_proxy *proxy, uint8_t flags, const uint8_t *body,
    ...
    if (message.topic_len >= sizeof topic) {
        proxy->stats.skipped++;
        mesh_log_debug("mqtt", "Dropped a message on a topic too long to forward (%zu bytes)",
                       message.topic_len);
        return true;
    }
...
static bool mqtt_handle(struct mesh_mqtt_proxy *proxy, const struct mesh_mqtt_header *header,
    ...
    case MESH_MQTT_UNSUBACK:
        ...
        mesh_log_debug("mqtt", "Ignoring an unexpected packet type %u", (unsigned)header->type);
        return true;
...
static bool mqtt_consume(struct mesh_mqtt_proxy *proxy) {
    ...
    for (;;) {
        ...
        if (header.remaining > sizeof proxy->in - header.header_len) {
            ...
            proxy->stats.skipped++;
            mesh_log_debug("mqtt", "Skipping a %zu-byte message", header.remaining);
            continue;
        }
...
void mesh_mqtt_proxy_set_ca_bundle(struct mesh_mqtt_proxy *proxy, const char *path) {
    ...
    (void)mesh_str_copy(proxy->ca_bundle, sizeof proxy->ca_bundle, path);
}
...
int mesh_mqtt_proxy_start(struct mesh_mqtt_proxy *proxy,
    ...
    proxy->last_error[0] = '\0';
    (void)mesh_str_copy(proxy->host, sizeof proxy->host, host);
...
void mesh_mqtt_proxy_tick(struct mesh_mqtt_proxy *proxy, uint64_t now_ms) {
     ...
     * `proxy->now_ms` rather than calling mesh_time_monotonic_ms() where it happens to be. That
...
const char *mesh_mqtt_proxy_state_string(enum mesh_mqtt_proxy_state state) {
    ...
    case MESH_MQTT_PROXY_OFF:
        return mesh_str(MESH_STR_MQTT_STATE_OFF);
    case MESH_MQTT_PROXY_RESOLVING:
        return mesh_str(MESH_STR_MQTT_STATE_RESOLVING);
    case MESH_MQTT_PROXY_CONNECTING:
        return mesh_str(MESH_STR_MQTT_STATE_CONNECTING);
    case MESH_MQTT_PROXY_SECURING:
        return mesh_str(MESH_STR_MQTT_STATE_SECURING);
    case MESH_MQTT_PROXY_GREETING:
        return mesh_str(MESH_STR_MQTT_STATE_GREETING);
    case MESH_MQTT_PROXY_READY:
        return mesh_str(MESH_STR_MQTT_STATE_READY);
    case MESH_MQTT_PROXY_WAITING:
    case MESH_MQTT_PROXY_STATE_COUNT:
    default:
        return mesh_str(MESH_STR_MQTT_STATE_WAITING);
    }
}
