...
static void install_fail(struct mesh_firmware_install *install,
                         enum mesh_firmware_install_error error) {
    inkwell_log_error("firmware", "The install failed: %s",
...
static void install_enter_waiting(struct mesh_firmware_install *install, uint64_t now_ms) {
    ...
    inkwell_log_info("firmware", "Waiting for a bootloader to enumerate");
}
...
static void install_begin_write(struct mesh_firmware_install *install,
    ...
    inkwell_log_info("firmware", "Writing %zu bytes of firmware to %s", install->image_len,
...
static void install_tick_writing(struct mesh_firmware_install *install, uint64_t now_ms) {
    ...
    inkwell_log_info("firmware", "Wrote %llu bytes; waiting for the board to restart",
                     (unsigned long long)install->write.total);
    install_enter_restarting(install, now_ms);
}
...
static void install_tick_restarting(struct mesh_firmware_install *install, uint64_t now_ms) {
    ...
    if (!still_in_dfu && install_radio_present(install)) {
         ...
         * two on a T114.
         */
        inkwell_log_info("firmware", "The board restarted into its new firmware");
...
int mesh_firmware_install_start(struct mesh_firmware_install *install, struct inkwell_loop *loop,
    ...
    size_t len = 0U;
    uint8_t *const image = inkwell_file_read(image_path, MESH_FIRMWARE_INSTALL_IMAGE_MAX, &len);
    if (image == NULL) {
        ...
        inkwell_log_error("firmware", "The staged image could not be read: %s", image_path);
        install_refuse(install, MESH_FIRMWARE_INSTALL_ERROR_UNAVAILABLE);
        return -EIO;
    }
    ...
    if (arm != NULL) {
        ...
        if (armed != 0) {
            inkwell_log_error("firmware", "The radio would not take the DFU request: %s",
                              strerror(-armed));
            install_refuse(install, MESH_FIRMWARE_INSTALL_ERROR_ARM);
            return armed;
        }
        install->state = MESH_FIRMWARE_INSTALL_ARMING;
        inkwell_log_info("firmware", "Asked the radio to enter DFU; %u blocks are ready",
                         (unsigned)install->uf2.blocks);
    } else {
        install->state = MESH_FIRMWARE_INSTALL_WAITING;
        inkwell_log_info("firmware", "Waiting for a bootloader; %u blocks are ready",
                         (unsigned)install->uf2.blocks);
    }
