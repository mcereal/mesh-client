...
struct mesh_ble_transport_state {
    ...
    /* The conversation itself. The link attaches to it while connected and feeds it every
       packet it reads. `protocol` is what the code uses; `own_session` is the Meshtastic session
       it falls back to when nothing was handed in (tests, --list-devices). */
    ...
    if (state != NULL && state->client_initialised) {
        ...
        if (state->link_state == MESH_BLE_LINK_CONNECTED) {
            ...
            /* BlueZ can hold a link up with nothing answering on the other end of it; the
               protocol is what notices, and dropping it is what lets auto-connect try. */
...
static int mesh_ble_drain_wake_callback(int fd, uint32_t events, void *userdata) {
    ...
    if (state != NULL && state->drain_pending &&
        inkwell_time_monotonic_ms() >= state->drain_retry_at_ms) {
        mesh_ble_drain_from_radio(state);
    }
    return 0;
}
...
static int mesh_ble_start(struct mesh_transport *transport, const struct mesh_app_config *config,
    ...
    if (init_result < 0) {
        ...
        inkwell_log_warn("ble", "Failed to initialise BlueZ client: %s", strerror(-init_result));
        state->state = MESH_BLE_STATE_IDLE;
        return 0;
    }
    ...
    if (state->state == MESH_BLE_STATE_READY && config->preferred_ble_device[0] != '\0') {
        inkwell_log_info("ble", "Attempting to connect to preferred device '%s'",
...
static void mesh_ble_stop(struct mesh_transport *transport) {
    ...
    mesh_ble_clear_write_queue(state);
    state->loop = NULL;
    inkwell_log_info("ble", "BLE transport stopped");
}
...
static void mesh_ble_set_protocol(struct mesh_transport *transport,
                                  const struct mesh_protocol *protocol) {
    if (transport == NULL || transport->state == NULL) {
        return;
    }
    ((struct mesh_ble_transport_state *)transport->state)->protocol =
        protocol != NULL ? *protocol : (struct mesh_protocol){NULL, NULL};
}
...
static const struct mesh_transport_ops k_ble_ops = {
    ...
    .tick = mesh_ble_tick,
    .set_protocol = mesh_ble_set_protocol,
...
static size_t mesh_ble_reload_devices(struct mesh_ble_transport_state *state) {
    ...
    if (roster_changed) {
        for (size_t i = 0; i < device_count; ++i) {
            inkwell_log_debug("ble", "  %s (%s) RSSI=%d", state->devices[i].name,
                              state->devices[i].address, (int)state->devices[i].rssi);
        }
        for (size_t i = 0; i < device_count; ++i) {
            inkwell_str_copy(state->logged[i].address, sizeof state->logged[i].address,
                             state->devices[i].address);
            inkwell_str_copy(state->logged[i].name, sizeof state->logged[i].name,
...
static int mesh_ble_queue_packet(struct mesh_ble_transport_state *state, const uint8_t *packet,
    ...
    if (state->write_queue_len >= MESH_BLE_MAX_OUTBOUND_PACKETS) {
        inkwell_log_warn("ble", "write queue full, dropping %zu byte packet", len);
        return -ENOSPC;
    }
...
static void mesh_ble_drain_from_radio(struct mesh_ble_transport_state *state) {
    ...
    for (size_t i = 0; i < MESH_BLE_READS_PER_TURN; ++i) {
        ...
        if (result < 0) {
            ...
            inkwell_log_warn("ble", "FromRadio read failed (%s); retrying in %" PRIu64 " ms",
        ...
        inkwell_log_debug("ble", "FromRadio packet (%zu bytes)", len);
        mesh_protocol_receive(&state->protocol, packet, len);
    }
...
static void mesh_ble_poll_connecting(struct mesh_ble_transport_state *state) {
     ...
     */
    if (result == -ETIMEDOUT) {
        inkwell_log_debug("ble", "%s: ServicesResolved poll timed out; reissuing",
                          state->connected_address);
        resolved = false;
    } else if (result < 0) {
        inkwell_log_warn("ble", "ServicesResolved query failed for %s (%d)",
    ...
    if (!resolved) {
        if (now - state->connect_started_ms >= MESH_BLE_SERVICES_TIMEOUT_MS) {
            inkwell_log_warn("ble", "%s: GATT services still unresolved after %u ms",
        ...
        if (!state->services_wait_logged) {
            inkwell_log_info("ble", "Link to %s is up; waiting for GATT service discovery",
                             state->connected_address);
...
static int mesh_ble_complete_connect(struct mesh_ble_transport_state *state) {
    ...
    state->bytes_received = 0U;
    inkwell_ble_process(&state->central);
    mesh_protocol_attach(&state->protocol, mesh_ble_session_send, state);
    int handshake_result = mesh_protocol_begin(&state->protocol);
    if (handshake_result < 0) {
        inkwell_log_warn("ble", "Failed to request config sync: %d", handshake_result);
    }
    inkwell_log_info("ble", "Connected to %s", address);
...
static void mesh_ble_reset_link(struct mesh_ble_transport_state *state, const char *reason) {
    ...
    state->scan_resume_at_ms = inkwell_time_monotonic_ms() + state->scan_resume_grace_ms;
    state->notifications_enabled = false;
    state->drain_pending = false;
    state->drain_retry_at_ms = 0U;
    state->drain_failures = 0U;
...
static void mesh_ble_poll_pairing(struct mesh_ble_transport_state *state) {
    ...
    if (poll == 0) {
        ...
        if (inkwell_time_monotonic_ms() - state->pair_started_ms >= MESH_BLE_PAIR_TIMEOUT_MS) {
    ...
    if (poll < 0 || pair_result < 0) {
        inkwell_log_warn("ble", "Pairing with %s failed (%d)", state->pairing_address, pair_result);
    ...
    state->last_refresh_ms = inkwell_time_monotonic_ms();

    if (then_connect) {
        int result = mesh_ble_do_connect(state, address, false);
        if (result < 0 && result != -EALREADY && result != -EINPROGRESS) {
            inkwell_log_warn("ble", "Connect after pairing with %s failed (%d)", address, result);
        }
    }
}
...
int mesh_ble_transport_forget(struct mesh_transport *transport, const char *address) {
    ...
    inkwell_log_info("ble", "Forgot %s", address);
...
int mesh_ble_transport_check_link(struct mesh_transport *transport) {
    ...
    if (result < 0) {
        ...
        if (result != -EAGAIN) {
            inkwell_log_debug("ble", "Could not read Device1.Connected: %d", result);
        }
        return result;
    }
    if (connected) {
    ...
    inkwell_log_warn("ble", "BlueZ reports %s disconnected", state->connected_address);
