...
static void load_node_state(struct mesh_ui_node_summary *node, const char *value) {
    ...
    if (mesh_ui_store_fields_read(value, fields, MESH_ARRAY_LEN(fields)) < 2U) {
        return;
    }
...
static void load_node_rssi(struct mesh_ui_node_summary *node, const char *value) {
    ...
    if (mesh_ui_store_fields_read(value, fields, MESH_ARRAY_LEN(fields)) < 1U) {
        return;
    }
    node->has_rssi = true;
    node->rx_rssi = rssi;
...
static void load_node_position(struct mesh_ui_node_summary *node, const char *value) {
    ...
    if (mesh_ui_store_fields_read(value, fields, MESH_ARRAY_LEN(fields)) < 7U) {
        return;
    }
...
static void load_node_neighbor(struct mesh_ui_node_summary *node, uint32_t slot,
    ...
    if (!cache_fields(value, fields, MESH_ARRAY_LEN(fields)) || entry.node_id == 0U) {
        return;
    }
