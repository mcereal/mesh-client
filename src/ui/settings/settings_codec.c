 ...
 *
 * The parse and print themselves are mesh/ui/form_codec.h's. What is this client's is which
...
void mesh_ui_settings_decimal_text(int64_t scaled, uint32_t held_digits, uint32_t shown_digits,
    ...
    mesh_ui_form_decimal_text(scaled, held_digits, shown_digits, out, out_len);
}

bool mesh_ui_settings_decimal_parse(const char *text, uint32_t digits, int64_t limit_whole,
                                    int64_t *out_scaled) {
    return mesh_ui_form_decimal_parse(text, digits, limit_whole, out_scaled);
}
...
void mesh_ui_settings_coord_text(int32_t value_i, char *out, size_t out_len) {
    mesh_ui_form_decimal_text(value_i, MESH_UI_COORD_DIGITS, 5U, out, out_len);
}

bool mesh_ui_settings_coord_parse(const char *text, int32_t limit_degrees, int32_t *out_i) {
    int64_t scaled = 0;
    if (out_i == NULL ||
        !mesh_ui_form_decimal_parse(text, MESH_UI_COORD_DIGITS, (int64_t)limit_degrees, &scaled)) {
        return false;
    }
...
void mesh_ui_settings_key_text(const uint8_t *key, size_t len, char *out, size_t out_len) {
    mesh_ui_form_bytes_base64(key, len, out, out_len);
}
...
bool mesh_ui_settings_key_parse(const char *text, uint8_t *out, size_t out_cap, size_t *out_len) {
    ...
    return mesh_ui_form_bytes_parse(text, k_key_sizes, sizeof k_key_sizes / sizeof k_key_sizes[0],
                                    out, out_cap, out_len);
}
